...
The following experiments perform `100_000` iterations and report the time for the complete batch.
The random-prefix experiment uses randomly generated IPv4 prefixes. The BGP-prefix experiment uses
all IPv4 prefixes exchanged at AMS-IX in the bundled snapshot, currently 1,058,306 prefixes. See
...
The memory benchmark (`cargo test --bench memory --release -- --nocapture`) stores all 1,058,306
AMS-IX IPv4 prefixes with 32-bit values. Both `PrefixMap` and `TreeBitMap` report 12.0 mB for that
map.
